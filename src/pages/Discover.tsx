...
const Discover = () => {
  ...
  if (isLoading) return <Loader title={t('discover.loadingTitle')} />;
    ...
    <div className="flex flex-col gap-10">
      <section className="glass-panel relative overflow-hidden rounded-[32px] border border-white/10 p-6 sm:p-8">
        ...
        <div className="relative z-10 flex flex-col gap-8 lg:flex-row lg:items-end lg:justify-between">
          <div className="max-w-3xl">
            <p className="text-label-sm uppercase tracking-[0.28em] text-primary-fixed">{t('discover.label')}</p>
            <h1 className="mt-3 text-display-xl text-white">{t('discover.title', { genre: genreTitle || '' })}</h1>
      ...
      <section className="glass-panel overflow-hidden rounded-[32px] border border-white/10 p-6 sm:p-8">
        <div className="mb-6 flex items-end justify-between gap-4">
          <div>
            <p className="text-label-sm uppercase tracking-[0.24em] text-primary-fixed">{t('discover.sectionLabel')}</p>
            <h2 className="mt-2 text-headline-lg text-white">{t('discover.sectionTitle')}</h2>
          </div>
          <p className="hidden text-sm text-on-surface-variant md:block">
            {t('discover.readyTracks', { count: tracks.length })}
          </p>
        </div>
