...
import { getDeezerTrack } from './_lib/providers/deezer';
...
export default async (request: Request, context: Context) => {
  try {
    ...
    const relatedTracks = await getLastFmSimilarTracks(
      track.artist?.name ?? '',
      track.title,
      getLimit(request, 10),
    );

    return jsonResponse(relatedTracks);
  } catch (error) {
